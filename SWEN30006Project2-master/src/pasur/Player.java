...
public abstract class Player
{
    ...
    /**
     * Calculates score for player at any given time, depending on previous score, and cards that are currently
     * in the players picked cards and sur pile
     * @param isRoundEnd True if at the end of round, false otherwise. If at end of round, must add score to running
     *                   total
     * @return Current score
     */
    public int getScore(boolean isRoundEnd)
    {
        if (!pickedCards.isEmpty() || surs.isEmpty()) {
            lastScore = ScoreStrategyFactory.getInstance().getCompositeScoreStrategy(this.pickedCards, this.surs).getScore();

           // return runningScore + lastScore;
        }
        if (isRoundEnd) {
            runningScore += lastScore;
            lastScore = 0;
        }
